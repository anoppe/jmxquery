...
public class JMXQuery
{
	...
	private String checkString, checkNotString;
	...
	private int report(PrintStream out)
	{
		...
		}else if (checkString != null && compare( checkString)){
			status = RETURN_CRITICAL;
			out.print(CRITICAL_STRING);
		}else if (checkNotString != null && !compare( checkNotString)){
			status = RETURN_CRITICAL;
			out.print(CRITICAL_STRING);
		}else{
			status = RETURN_OK;
